...
public class SplashmoneyDirConfig
  extends JDialog 
  implements ActionListener 
{
  ...
  public SplashmoneyDirConfig(Main ext, Resources rr, RootAccount root) {
    ...
    dirField = new JTextField(root.getParameter("PalmSync.qifdir", "."), 30);

    int y = 0;
    JPanel p = new JPanel(new GridBagLayout());
    p.add(new JTextPanel(rr.getString("select_splashdir")),
          AwtUtil.getConstraints(0,y++,1,0,3,1,true,true));
    p.add(Box.createVerticalStrut(12),
          AwtUtil.getConstraints(0,y++,0,0,3,1,true,true));
    p.add(new JLabel(rr.getString("splashqif_dir")+": ", JLabel.RIGHT),
    ...
    setSize(minmax(200, sz.width, 600), minmax(200, sz.height, 600));
  ...
  private void okPressed() {
    root.setParameter("PalmSync.qifdir", dirField.getText());
    wasCanceled = false;
    setVisible(false);
    dispose();
  }
