...
public enum Header implements HeaderConstants, Strings {
    ...
    PAYMENT("Payment*", "getTotalDebtPayment", "Calculated payments are estimated (based on Account's Payment Plan settings)",
    ...
    BALANCE("Balance", "getTotalDebtBalance", "Current Balance",
            SortView.OFF, SwingConstants.RIGHT,
...
//	private Object[] params = null;
    ...
    public TableHeaderListener getListener(JComponent comp, Header header, DebtViewPanel viewPanel) {
        if (this.listener == null && listenerClass != null) {
            ...
            } catch (IllegalArgumentException | NoSuchMethodException | InstantiationException | InvocationTargetException | SecurityException | IllegalAccessException e) {
    ...
    public int compareAccounts(Account a1, Account a2, BalanceType bType, CreditLimitType cType) {
        Util.logConsole(true, String.format("Inside: compareAccounts(): Acct: %s <> %s bType: %s cType: %s", a1, a2, bType, cType));

        //        if (bType == null) bType = BalanceType.BALANCE;
//        if (bType == null) bType = Main.getWidgetCalculationBalanceTypeChoiceAsBalanceType();
        if (bType == null) {Util.logConsole(">> WARNING! bType is null... Why?");}

        if (cType == null) cType = CreditLimitType.CREDIT_LIMIT;
        Util.logConsole(true, String.format(".... after......           Acct: %s <> %s bType: %s cType: %s", a1, a2, bType, cType));
        long tmp = 0;
        switch (this) {
            case HIERARCHY:
            case NAME:
            ...
            case INTEREST:
                ...
                return 0;
            case BALANCE:
                tmp = AccountUtils.getActiveXBalance(a1, bType) - AccountUtils.getActiveXBalance(a2, bType);
                if (tmp < 0) return -1;
                if (tmp > 0) return 1;
                return 0;
            case BAL_FLG:
