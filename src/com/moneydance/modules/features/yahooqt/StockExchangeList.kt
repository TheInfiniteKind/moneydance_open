 ...
 * derived from [
 * http://www.wikinvest.com/wiki/List_of_Stock_Exchanges](http://www.wikinvest.com/wiki/List_of_Stock_Exchanges) or
 * [
 * http://en.wikipedia.org/wiki/List_of_stock_exchanges](http://en.wikipedia.org/wiki/List_of_stock_exchanges).
 *
 *
 * Yahoo exchange list is currently here: [
 * http://finance.yahoo.com/exchanges](http://finance.yahoo.com/exchanges). Some stuff was found for Google here:
 * [
 * http://directory.google.com/Top/Business/Investing/Stocks_and_Bonds/Exchanges/](http://directory.google.com/Top/Business/Investing/Stocks_and_Bonds/Exchanges/), however
...
class StockExchangeList {
   ...
   */
  private val _exchangeList: MutableMap<String?, StockExchange> = TreeMap()
  ...
  fun save(): Boolean {
    var success = configFile
    if (success) success = saveToFile()
    return success
  }
  
  fun contains(exchangeId: String?): Boolean {
    return _exchangeList.containsKey(exchangeId)
  }
  
  fun getById(exchangeId: String?): StockExchange {
    val exchange = _exchangeList[exchangeId] ?: return StockExchange.DEFAULT
    return exchange
  }
  
  val fullList: List<StockExchange>
    get() {
      val sortedList = ArrayList(_exchangeList.values)
      Collections.sort(sortedList)
      return Collections.unmodifiableList(sortedList)
    }
  
  fun findByGooglePrefix(prefix: String): StockExchange? {
    if (isBlank(prefix)) return null
    val matches: MutableList<StockExchange> = ArrayList()
    for (exchange in _exchangeList.values) {
      if (prefix.compareTo(exchange.symbolGoogle!!) == 0) matches.add(exchange)
    }
    ...
    for (candidate in matches) {
      if (candidate.priceMultiplier != 1.0) return candidate
    }
    return matches[0]
  }
  
  fun findByYahooSuffix(suffix: String): StockExchange? {
    if (isBlank(suffix)) return null
    val matches: MutableList<StockExchange> = ArrayList()
    for (exchange in _exchangeList.values) {
      if (suffix.compareTo(exchange.symbolYahoo!!) == 0) matches.add(exchange)
    }
    if (matches.isEmpty()) return null // not found
  ...
  private val configFile: Boolean
    /**
    ...
    get() {
      if (_configFile == null) {
        _configFile = File(Common.getDataRootDirectory(), "stockExchanges.dict")
      }
  ...
  private fun saveToFile(): Boolean {
    ...
    settings[STOCK_EXCHANGES_KEY] = exchangeList
    try {
      settings.writeToFile(_configFile)
      success = true
    } catch (e: StringEncodingException) {
      System.err.println("Quotes and Exchange Rates plugin error writing to file: " + _configFile!!.absolutePath)
      e.printStackTrace()
    } catch (e: IOException) {
      System.err.println("Quotes and Exchange Rates plugin error writing to file: " + _configFile!!.absolutePath)
      e.printStackTrace()
  ...
  private fun loadFromFile(): Boolean {
    ...
    try {
      ...
      val value = settings[STOCK_EXCHANGES_KEY]
      if ((value == null) || value !is StreamVector) return false
      val exchanges = value
      if (exchanges.isEmpty()) return false
      for (exchangeSetting in exchanges) {
        if (exchangeSetting !is StreamTable) continue
        val exchange = StockExchange()
        exchange.loadFromSettings(exchangeSetting)
        if (_exchangeList.containsKey(exchange.exchangeId)) {
          System.err.println(
            "Quotes and Exchange Rates plugin found duplicate stock exchange " +
            "entry name: " + exchange.name + " in file: " + _configFile!!.absolutePath
          )
        } else {
          _exchangeList[exchange.exchangeId] = exchange
        }
      }
      return true
    } catch (e: StringEncodingException) {
      System.err.println(
        "Quotes and Exchange Rates plugin error reading exchange list file: " +
        _configFile!!.absolutePath
      )
    } catch (e: IOException) {
      System.err.println(
        "Quotes and Exchange Rates plugin error reading exchange list file: " +
  ...
  private val defaultExchangeTable: Unit
    /**
     ...
     */
    get() {
      val exchanges: MutableList<StockExchange> = ArrayList()
  ...
  companion object {
    ...
    const val STOCK_EXCHANGES_KEY: String = "exchangeList"
  }
}
