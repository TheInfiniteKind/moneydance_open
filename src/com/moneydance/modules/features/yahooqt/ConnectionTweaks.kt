/*
 * Copyright (c) 2025 The Infinite Kind, Limited
 */
...
object ConnectionTweaks {
  
  private var userAgentIndex = -1
  private var nextUserAgentChangeTimestamp:Long = 0
  
  /**
   * Return a random http user-agent string which changes every 24 hours
   */
  val rotatingUserAgent:String
    get() {
      if(nextUserAgentChangeTimestamp <= 0 || nextUserAgentChangeTimestamp < System.currentTimeMillis()) {
        userAgentIndex = Random.nextInt(0, userAgentStrings.size)
        nextUserAgentChangeTimestamp = System.currentTimeMillis() + 1000 * 60 * 60 * 24
      }
      return userAgentStrings[userAgentIndex]
    }
  
  
  private val userAgentStrings = 
    listOf("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.6367.91 Safari/537.36",
